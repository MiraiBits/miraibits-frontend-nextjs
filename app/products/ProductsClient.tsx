...
export default function ProductsClient({ initialProducts, initialQuery }: Props) {
  ...
  useEffect(() => {
    try {
      const savedCols = Number(localStorage.getItem('products_cols') || String(DEFAULT_COLS));
      if (AVAILABLE_COLS.includes(savedCols as any)) setCols(savedCols);
    } catch {}
  }, []);
  useEffect(() => { try { localStorage.setItem('products_cols', String(cols)); } catch {} }, [cols]);
  ...
  function ColumnsIcon({ columns, active }: { columns: 2 | 3 | 4; active: boolean }) {
    ...
    const cellClass = active ? `${baseCell} bg-gray-900` : `${baseCell} bg-gray-400`;
    const gridCols = columns === 2 ? 'grid-cols-2' : columns === 3 ? 'grid-cols-3' : 'grid-cols-4';
    const wrapperClass = `grid ${gridCols} gap-0.5 p-1 rounded-md border ${active ? 'bg-gray-100 border-gray-200' : 'bg-transparent border-gray-200'}`;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input
            ...
            className="h-10 w-64 border border-gray-200 rounded-md px-3 text-sm focus:outline-none focus:ring-2 focus:ring-sakura-200"
          />
          {(
            <div className="flex items-center gap-2">
              ...
              {[2,3,4].map(option => (
                ...
                >
                  <ColumnsIcon columns={option as 2 | 3 | 4} active={cols === option} />
                </button>
              ))}
            </div>
          )}
